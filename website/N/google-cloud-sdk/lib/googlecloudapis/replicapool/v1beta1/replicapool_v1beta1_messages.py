...
class Action(messages.Message):
  ...
  Fields:
    ...
    envVariables: A list of environment variables to use for the commands in
      this action.
    timeoutMilliSeconds: If an action on a particular virtual machine does not
      finish in the timeoutMS, the current command is failed and the replica
      is considered a failure. No further actions are executed. The command is
      killed by sending a "kill" signal to the shell process that is running
      the command. No additional efforts are made to stop any processes that
      were spawned or created as the result of running the action. The default
      is the max allowed value, 1 hour (i.e. 3600000 milliseconds).
  """
...
class HealthCheck(messages.Message):
  ...
  Fields:
    ...
    timeoutSec: How long (in seconds) to wait before claiming failure for this
      healthcheck. The default value is 5 seconds.
...
class ReplicapoolPoolsGetRequest(messages.Message):
  ...
  Fields:
    poolName: The name of the replica pool for which you want to get more
      information.
    projectName: The project ID for this replica pool.
    zone: The zone for this replica pool.
  """
...
class ReplicapoolPoolsResizeRequest(messages.Message):
  ...
  Fields:
    numReplicas: The desired number of replicas to resize to. If this number
      ...
      This is an asynchronous operation, and multiple overlapping resize
      requests can be made. Replica Pools will use the information from the
      last resize request.
...
class ReplicapoolReplicasGetRequest(messages.Message):
  ...
  Fields:
    ...
    replicaName: The name of the replica for which you want to get more
      information.
    zone: The zone where the replica lives.
  """
...
class Template(messages.Message):
  ...
  Fields:
    action: A list actions you want to run during initialization of your
      replicas. Actions are run as shell commands and are executed one after
      the other in the same bash shell, so any state established by one
    ...
    version: A free-form string describing the version of this template. You
      ...
      or template-v1.
    vmParams: The virtual machine parameters to use for these replicas. You
      can define settings such as the machine type and the image of these
      replicas. This is required if replia type is SMART_VM
  """
...
class VmParams(messages.Message):
  """A Compute Engine Instance resource. Identical to the corresponding
