...
class DatabaseFlags(messages.Message):
  ...
  Fields:
    name: The name of the flag. These flags are passed at instance startup, so
      ...
      event_scheduler on or off (Note: The event scheduler will only work
      reliably if the instance activationPolicy is set to ALWAYS.) general_log
      on or off group_concat_max_len 4..17179869184
      innodb_flush_log_at_trx_commit 0..2 innodb_lock_wait_timeout
      1..1073741824 log_bin_trust_function_creators on or off log_output Can
      be either TABLE or NONE, FILE is not supported.
...
class DatabaseInstance(messages.Message):
  ...
  ipAddresses = messages.MessageField('IpMapping', 5, repeated=True)
  kind = messages.StringField(6, default=u'sql#instance')
  maxDiskSize = messages.IntegerField(7)
  project = messages.StringField(8)
  region = messages.StringField(9)
  serverCaCert = messages.MessageField('SslCert', 10)
  settings = messages.MessageField('Settings', 11)
...
class Settings(messages.Message):
  ...
  Fields:
    ...
    locationPreference: The location preference settings. This allows the
      instance to be located as near as possible to either an AppEngine app or
      GCE zone for better perfomance.
  ...
  ipConfiguration = messages.MessageField('IpConfiguration', 5)
  kind = messages.StringField(6, default=u'sql#settings')
  locationPreference = messages.MessageField('LocationPreference', 7)
  pricingPlan = messages.StringField(8)
  replicationType = messages.StringField(9)
  settingsVersion = messages.IntegerField(10)
  tier = messages.StringField(11)
