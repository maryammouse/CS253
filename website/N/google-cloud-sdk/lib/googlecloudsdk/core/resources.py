...
class UnknownFieldException(UserError):
  ...
  def __init__(self, expected):
    super(UnknownFieldException, self).__init__(
        'unknown field [{expected}]'.format(expected=expected))
...
class _ResourceParser(object):
  ...
  def ParseCollectionPath(self, collectionpath, kwargs):
    ...
    for param, field in zip(self.method_config.ordered_params, fields):
      if field:
        ...
        # The param wasn't specified in the path or in the kwargs, so call the
        # registered default.
        api_collection_funcs = self.registry.default_param_funcs.get(param)
        if not api_collection_funcs:
          raise UnknownFieldException(param)
        api, collection = self.collection.split('.')
        collection_funcs = api_collection_funcs.get(api)
        if not collection_funcs:
          raise UnknownFieldException(param)
        if collection in collection_funcs:
          params[param] = collection_funcs[collection]()
        elif None in collection_funcs:
          params[param] = collection_funcs[None]()
        else:
          raise UnknownFieldException(param)
    ...
    self_link = self.client.url + util.ExpandRelativePath(
        self.method_config, params)

    resource = Resource(self.collection, request, self_link,
                        self.method_config.ordered_params)
    return resource
...
class Resource(object):
  ...
  def __init__(self, collection, request, self_link, ordered_params):
...
class _Registry(object):
  ...
  def SetParamDefault(self, api, collection, param, func):
    ...
    Args:
      ...
      func: func()->str, A function that returns a string or raises an
          exception that tells the user how to fix the problem.
    ...
    collection_funcs[collection] = func
  ...
  def ParseURL(self, url, collection):
    ...
    resource = parser.ParseCollectionPath(None, params)
...
def SetParamDefault(api, collection, param, func):
  """Provide a function that will be used to fill in missing values.
  ...
  Args:
    ...
    func: func()->str, A function that returns a string or raises an
        exception that tells the user how to fix the problem.
  """
  _REGISTRY.SetParamDefault(api, collection, param, func)
...
def ParseResource(line, kwargs, collection=None):
  ...
  if line.startswith('https://'):
    ...
    return _REGISTRY.ParseCollectionPath(collection, line, kwargs)
